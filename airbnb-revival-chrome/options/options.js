document.addEventListener('DOMContentLoaded', async () => {
	...
	if (apiKey) {
	  // Show only the first few and last few characters of the API key for security
	  const maskedKey = apiKey.substring(0, 6) + '...' + apiKey.substring(apiKey.length - 4);
	  apiKeyInput.value = maskedKey;
	  apiKeyInput.setAttribute('data-has-saved-key', 'true');
	}

	// When the input field is clicked and has a saved key, clear it so user can enter a new one
	...
	saveButton.addEventListener('click', async () => {
	  ...
	  try {
		...
		await browser.storage.local.set({ apiKey: newApiKey });

		// Mask the key in the input field
		const maskedKey = newApiKey.substring(0, 6) + '...' + newApiKey.substring(newApiKey.length - 4);
		apiKeyInput.value = maskedKey;
		apiKeyInput.setAttribute('data-has-saved-key', 'true');
